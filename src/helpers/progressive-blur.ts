...
const getApplicableStylesForStep = (step: number, steps: number[], blurAmount: number) => {
	const gradientOffsets = [`rgba(0, 0, 0, 0) ${steps[step]}%`, `rgba(0, 0, 0, 1) ${steps[step + 1]}%`]
	if (steps[step + 2]) {
	...
	return {
		'backdrop-filter': `blur(${blurAmount * 2 ** step}px)`,
...
const getStyleString = (styles: CSSProperties) => {
...
const getCssString = (styles: CSSProperties) => {
